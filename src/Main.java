...
public class Main {
    public static void main(String[] args) {
        ...
        // Loại bỏ các ký tự đặc biệt và chuyển tất cả các từ thành chữ thường
        String[] words = sentence.replaceAll("[^a-zA-Z ]", "").toLowerCase().split("\\s+");

        // Tạo một Map để lưu trữ từng từ và số lần xuất hiện
        Map<String, Integer> wordCountMap = new HashMap<>();

        // Đếm số lần xuất hiện của từng từ
        for (String word : words) {
            if (wordCountMap.containsKey(word)) {
                // Tăng số lần xuất hiện lên 1 nếu từ đã tồn tại trong Map
                wordCountMap.put(word, wordCountMap.get(word) + 1);
            } else {
                // Nếu từ chưa tồn tại trong Map, thêm vào Map với số lần xuất hiện là 1
